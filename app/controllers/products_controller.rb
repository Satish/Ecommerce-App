class ProductsController < ApplicationController
  ...
  def index
    ...
    @products = @store.products.published.search( params[:query], options )
    render_products
  end
  ...
  def update_select_options
    product = @store.products.published.find_by_id(params[:id])
  ...
  def find_product
    @product = @store.products.published.find_by_permalink(params[:permalink])
    redirect_to products_path and flash[:error] = PAGE_NOT_FOUND_ERROR_MESSAGE and return unless @product
  end
